<template><div><h1 id="redis-深度历险-核心原理和应用实践" tabindex="-1"><a class="header-anchor" href="#redis-深度历险-核心原理和应用实践"><span>Redis 深度历险：核心原理和应用实践</span></a></h1>
...
<li><a href="#Inset%E5%B0%8F%E6%95%B4%E6%95%B0%E9%9B%86%E5%90%88">Inset 小整数集合</a></li>
...
<li><a href="#%E6%B8%90%E8%BF%9B%E5%BC%8Frehash">渐进式 rehash</a></li>
...
<li><a href="#Redis%E7%BA%BF%E7%A8%8B%E6%A8%A1%E5%9E%8B">Redis 线程模型</a>
<ul>
<li><a href="#%E9%9D%9E%E9%98%BB%E5%A1%9EIO">非阻塞 IO</a></li>
<li><a href="#%E4%BA%8B%E4%BB%B6%E8%BD%AE%E8%AF%A2">事件轮询</a></li>
</ul>
</li>
<li><a href="#Redis%E9%80%9A%E4%BF%A1%E5%8D%8F%E8%AE%AE">Redis 通信协议</a>
<ul>
...
<ul>
<li><a href="#CAP%E7%90%86%E8%AE%BA">CAP 理论</a></li>
<li><a href="#Redis%E7%9A%84%E4%B8%BB%E4%BB%8E%E6%95%B0%E6%8D%AE%E4%B8%80%E8%87%B4%E6%80%A7">Redis 的主从数据一致性</a></li>
...
<li><a href="#info%E6%8C%87%E4%BB%A4">info 指令</a>
<ul>
...
<li><a href="#LRU">LRU</a>
<ul>
<li><a href="#%E4%BC%A0%E7%BB%9FLRU%E7%AE%97%E6%B3%95">传统 LRU 算法</a></li>
<li><a href="#Redis%E5%B0%8F%E6%94%B9LRU%E7%AE%97%E6%B3%95">Redis 小改 LRU 算法</a></li>
</ul>
</li>
</ul>
</li>
...
<li><a href="#%E5%BC%82%E6%AD%A5AOF">异步 AOF</a></li>
</ul>
</li>
</ul>
...
<p>Redis 的字符串属于动态字符串，是可以修改的字符串，内部结构实现上类似于 Java 的 ArrayList，采用预分配冗余空间的方式来减少内存的频繁分配。</p>
<p>分配方式：</p>
<ol>
<li>当字符串小于 1M 时，扩容都是加倍现有的空间</li>
<li>如果超过 1M 时，扩容时一次只会多扩 1M 的空间</li>
</ol>
<p>字符串的最大长度为 512M</p>
...
<li>过期和 set 命令拓展<div class="language-bash line-numbers-mode" data-ext="sh" data-title="sh"><pre v-pre class="language-bash"><code><span class="token number">127.0</span>.0.1:637<span class="token operator"><span class="token file-descriptor important">9</span>></span> <span class="token builtin class-name">set</span> name <span class="token builtin class-name">test</span>
OK
<span class="token number">127.0</span>.0.1:637<span class="token operator"><span class="token file-descriptor important">9</span>></span> get name
...
<p>Redis 中的字符串是可以修改的字符串，在内存中它是以字节数组的形式存在的。Redis 的字符串叫着 SDS，也就是 Simple Dynamic String，它的结构是一个带长度信息的字节数组。</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>capacity 表示所分配数组的长度，len 表示字符串的实际长度，而 SDS 结构使用了范型 T，为什么不直接用 int 呢，这是因为当字符串比较短时，len 和 capacity 可以使用 byte 和 short 来表示，Redis 为了对内存做极致的优化，不同长度的字符串使用不同的结构体来表示。</p>
...
<p>为什么是 44 呢？首先来了解一下 Redis 对象头的结构体，所有的 Redis 对象都有这个结构体</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>embstr 和 raw 的存储方式不同，对于 embstr 来说只需要分配一次内存空间即可，但是 raw 却需要分配两次，为什么会这样呢？这跟内存分配的方式有关系，内存分配分配的方式都得是 2 的次方级，2，4，8，16，32，64 等，为了能够容纳 embstr 类型，最少也要 32 位的（因为头部+最小 SDS = 16+3 = 19），如果再大点就 64 位了，再大就会使用 raw 的存储方式了。所以 64 位是 embstr 的最大限度了。</p>
<p>那么 64-19 = 45，即 45 就是我们能够存储的真实字符串的最大字节数了，但是为什么是 44 呢？这是因为 C 中字符串是以字节\0 结尾的字符串，之所以多出这样一个字节，是为了便于直接使用 glibc 的字符串处理函数，以及为了便于字符串的调试打印输出。综上所述，embstr 类型的字符串的最大容纳就是 44 字节。</p>
...
<p>Redis 的列表相当于 Java 中的 LinkedList，是链表，意味着插入删除快，查询慢，当列表弹出了最后一个元素之后，该数据结构自动被删除，内存被回收。</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>entries 中的 entry 是一个元素类型，元素内容和元素前指针的描述</p>
<ul>
<li>
<p>prevlen </p>
<p>prevlen 字段表示前一个 entry 的字节长度，当压缩列表倒着遍历时，需要通过这个字段来快速定位到下一个元素的位置。它是一个变长的整数，当字符串长度小于 254(0xFE) 时，使用一个字节表示；如果达到或超出 254(0xFE) 那就使用 5 个字节来表示。第一个字节是 0xFE(254)，剩余四个字节表示字符串长度。</p>
</li>
<li>
<p>encoding</p>
<p>encoding 字段存储了元素内容的编码类型信息，ziplist 通过这个字段来决定后面的 content 内容的形式，Redis 通过 encoding 来标志 content 的数据类型</p>
<ol>
...
<li>1111xxxx 表示极小整数，xxxx 的范围只能是 (0001~1101), 也就是 1~13，因为 0000、1110、1111 都被占用了。读取到的 value 需要将 xxxx 减 1，也就是整数 0~12 就是最终的 value。</li>
</ol>
</li>
<li>
<p>content</p>
...
<p>因为 ziplist 都是紧凑存储，没有冗余空间意味着插入一个新的元素就需要调用 realloc 扩展内存。取决于内存分配器算法和当前的 ziplist 内存大小，realloc 可能会重新分配新的内存空间，并将之前的内容一次性拷贝到新的地址，也可能在原有的地址上进行扩展，这时就不需要进行旧内容的内存拷贝。如果 ziplist 占据内存太大，重新分配内存和拷贝内存就会有很大的消耗。所以 ziplist 不适合存储大型字符串，存储的元素也不宜过多。</p>
<h5 id="inset-小整数集合" tabindex="-1"><a class="header-anchor" href="#inset-小整数集合"><span>Inset 小整数集合</span></a></h5>
<p>当 set 集合容纳的元素都是整数并且元素个数较小时，Redis 会使用 intset 来存储结合元素。intset 是紧凑的数组结构，同时支持 16 位、32 位和 64 位整数。intset 基于整数数组来实现，并且具备长度可变、有序等特征。</p>
<h5 id="快速列表" tabindex="-1"><a class="header-anchor" href="#快速列表"><span>快速列表</span></a></h5>
<p>Redis 早期版本的列表仅支持 ziplist 和 linkedlist，数据较少的时候使用 ziplist，多的时候则使用 linkedlist。</p>
...
<span class="token punctuation">}</span>
 linkedlist结构
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>但是 linkedlist 的指针的附加空间相对起来太高了（16 字节）， 每个节点的内存都是单独分配，会加剧内存的碎片化，影响内存管理效率。后续版本对 ziplist 跟 linkedlist 做了优化，将两者进行了融合，就出现了 quicklist 的数据结构了。 </p>
<p>这是一个很有意思的数据结构，他将 ziplist 和 linkedlist 优势都结合起来了。它 将 linkedlist 按段切分，每一段使用 ziplist 来紧凑存储，多个 ziplist 之间使用双向指针串接起来。为了进一步节约空间，Redis 还会对 ziplist 进行压缩存储，使用 LZF 算法压缩，可以选择压缩深度。</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>quicklist 内部默认单个 ziplist 长度为 8k 字节，超出了这个字节数，就会新起一个 ziplist。ziplist 的长度由配置参数 <strong>list-max-ziplist-size</strong> 决定。</p>
...
<p>在实现上跟 Java 中的 HashMap 是相同的，一样的数组+链表的二维结构，不同的是 hash 的 value 只允许是字符串类型的，另外当 hash 较大时，会进行 rehash 操作，Java rehash 是一次性 rehash，是一个非常耗时的操作，而 Redis 则是采用渐进式的 rehash 策略。</p>
...
<p>在深入了解 hash 结构前，要先了解 Redis 这么多数据结构中出现最频繁的一个数据结构 dict</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>在 dict 内部有两个 hashtable，一般来说，只会有一个 hashtable 有值，在 dict 扩容缩容时，需要分配新的 hashtable，然后进行渐进式搬迁，这时候两个 hashtable 存 储的分别是旧的 hashtable 和新的 hashtable。待搬迁结束后，旧的 hashtable 被删除，新的 hashtable 取而代之。</p>
<figure><img src="@source/reading/image/image_nEnfUFny-j.png" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure>
<p>而 hashtable 其实跟 Java 中的 hashMap 很像，解决 hash 冲突的方式也是通过分桶链表的方式解决的。</p>
...
<span class="token punctuation">}</span>
 hashtable数据结构
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h5 id="渐进式-rehash" tabindex="-1"><a class="header-anchor" href="#渐进式-rehash"><span>渐进式 rehash</span></a></h5>
<p>大字典的扩容是比较耗时间的，需要重新申请新的数组，然后将旧字典所有链表中的元素重新挂接到新的数组下面，这是一个 O(n)级别的操作，这是单线程的 Redis 难以承受的，所以 Redis 使用渐进式 rehash 小步搬迁。</p>
<p>以下代码，就是向 Redis 的 hash 添加数据时的代码，其中 dictIsRehashing 是判断是否在进行 rehash，如果是则进行一次_dictRehashStep 操作，表示一次 rehash 操作，如果完成了就将 ht 指向 ht[1]，即新的 hash 表了。</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>从上面代码不难看出，其实数据的 rehash 操作是基于我们向客户端发出命令 hset/hdel 指令来触发 rehash 操作的，但是如果我们不发命令呢？Redis 就置之不理了吗？Of Course No！！！Redis 还会在定时任务中对字典进行主动搬迁。</p>
...
<p>当 hash 表因为元素的逐渐删除变得越来越稀疏时，Redis 会对 hash 表进行缩容来减少 hash 表的第一维数组空间占用。缩容的条件是元素个数低于数组长度的 10%。缩容不会考虑 Redis 是否正在做 bgsave。</p>
...
<p>与 Java 的 HashSet 特性相似，内部的键值对都是无序的唯一的，内部实现相当一个特殊的字典，字典中所有的 value 都是一个值 NULL</p>
...
<p>对于 zset 来说 ， 一方面它是一个 set，保证了内部 value 的唯一性，另一方面它可以给每个 value 赋予一个 score，代表这个 value 的排序权重。它的内部实现用的是一种叫着跳跃列表的数据结构。</p>
...
<p>zset 是一个复合结构，一方面它需要一个 hash 结构来存储 value 和 score 的 对应关系，另一方面为了满足提供按照 score 来排序的功能，还能够指定 score 的范围来获取 value 列表的功能，Redis 给出了一种数据结构，跳跃列表来实现这部分功能</p>
...
<p>对于 Redis 提供的容器型数据结构都会符合以下规则</p>
<ol>
<li>
...
<p>如果一个字符串已经设置了过期时间，然后你调用了 set 方法修改了它，它的过期时间会消失，根据**ttl **命令可以查看对象的过期时间。</p>
<h2 id="redis-线程模型" tabindex="-1"><a class="header-anchor" href="#redis-线程模型"><span>Redis 线程模型</span></a></h2>
<p>Redis 是一个单线程模型，却能有着不错的高并发抗压能力，这是怎么做到的呢？首先当然是因为 Redis 的内存读写操作，使得 Redis 能够用户不错的写入读取速度。</p>
<p>二则是这节的重点，Redis 的线程模型</p>
<h3 id="非阻塞-io" tabindex="-1"><a class="header-anchor" href="#非阻塞-io"><span>非阻塞 IO</span></a></h3>
<p>当我们调用套接字的读写方法，默认它们是阻塞的，比如 read 方法要传递进去一个参数 n，表示读取这么多字节后再返回，如果没有读够线程就会卡在那里，直到新的数据到来或者连接关闭了，read 方法才可以返回，线程才能继续处理。而 write 方法一般来说不会阻塞，除非内核为套接字分配的写缓冲区已经满了，write 方法就会阻塞，直到缓存区中有空闲空间挪出来了。</p>
...
<p>关于 IO 多路复用，下面这篇掘金博文讲得是非常不错的</p>
...
<p>Redis 是基于操作系统提供的 select 函数来处理事件监听和事件分发的，对于 select 函数输入是读写描述符列表 read_fds &amp; write_fds，输出是与之对应的可读可写事件。同时还提供了一个 timeout 参数，如果没有任何事件到来，那么就最多等待 timeout 时间，线程处于阻塞状态。一旦期间有任何事件到来，就可以立即返回。时间了之后还是没有任何事件到来，也会立即返回。拿到事件后，线程就可以继续挨个处理相应的事件。处理完了继续过来轮询。</p>
...
<h2 id="redis-通信协议" tabindex="-1"><a class="header-anchor" href="#redis-通信协议"><span>Redis 通信协议</span></a></h2>
...
<p>Reids 采用 fork 子进程的方式来帮助父进程分担压力，原因在于子进程和父进程是可以共享内存里面的代码段和数据段的。</p>
<p>但是随之而来的问题是：</p>
<p>子进程做数据持久化，但是父进程也是随时需要接收客户端请求修改数据的呀~，这种情况如何同步呢？</p>
<p>Redis 采用 COW 写时复制机制来解决这个问题，其实就是</p>
...
<p>子进程进行持久化后的数据段从那一刻就不变动了，所以 Redis 的持久化也叫快照。</p>
<h3 id="aof-重写" tabindex="-1"><a class="header-anchor" href="#aof-重写"><span>AOF 重写</span></a></h3>
...
<p>那么如果突然宕机，AOF 日志可能还没来得及完全刷到盘中，这个时候便会丢失了部分数据了，为了解决这个问题，我们可以使用 fsync 指令来将指定文件的内容强制从内核缓存刷到磁盘。只要 Redis 进程实时调用 fsync 函数就可以保证 aof 日志不丢失。</p>
<p>但是 fsync 是一个磁盘 IO 操作指令，很慢，如果实时地执行，那必定会对 Redis 的性能造成很大的影响，所以在生产环境的服务器中，Redis 通常是每隔 1s 左右执行一次 fsync 操作，周期 1s 是可以配置的。这是在数据安全性和性能之间做了一个折中，在保持高性能的同时，尽可能使得数据少丢失</p>
...
<p>Redis 提供的两种持久化方式都有其缺点，RDB 会丢失数据，AOF 如果恢复的数据量庞大，会导致恢复速度过慢而影响启动速度，Redis 4.0 为了解决这个问题，带来了一个新的持久化选项——混合持久化。将 rdb 文 件的内容和增量的 AOF 日志文件存在一起。这里的 AOF 日志不再是全量的日志，而是自持久化开始到持久化结束的这段时间发生的增量 AOF 日志，通常这部分 AOF 日志很小。</p>
...
<p>管道根本上其实是客户端提供的能力，通过将客户端发送的指令列表调整其读写顺序来节省 IO 消耗的时间，管道中的指令越多，效果越好。</p>
<p>想要更加深入了解管道的知识，我们先得从 Redis 的消息交互开始</p>
<p>首先是一次 Redis 的消息交互，无非是客户端将请求传送给服务器，服务器处理完毕后，再将响应回复给客户端，这要花费一个网络数据包来回的时间。</p>
<figure><img src="@source/reading/image/image_VsgVfjlpED.png" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure>
<p>如果是多次交互，则是发送多个 request 和回复多个 response，这就要耗费多次网络数据包来回的时间了。</p>
<p>对于客户端来说，我们可以理解为发送 request 就是写消息至管道，再从管道读消息为一次消息交互，所以一次消息交互其实就是 w-r，如果是多次就是 w-r-w-r ... 。但是通过管道的消息调整之后消息会变为 w-w-r-r 了，这有什么不一样的，这样其实我们就可以相当于一次性写，最后一次性读了，跟 w-r-w-r 相比，w-w-r-r 只需要耗费一次 IO，即花费一个网络数据包来回的时间了。</p>
<figure><img src="@source/reading/image/image_olXiLHYXSo.png" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure>
<p>对于服务端而言，还是一样的，有消息就处理，处理完就回复响应，而客户端通过管道的消息调整大幅降低了 IO 所花费的时间。</p>
<h3 id="管道压力测试" tabindex="-1"><a class="header-anchor" href="#管道压力测试"><span>管道压力测试</span></a></h3>
<p>通过 redis-benchmark 这个 Redis 自带的压力测试工具可以对管道的性能进行压测，其中-P 是参数是指单个管道内并发的请求数</p>
...
<p>而 read 操作则是需要等待读缓冲有数据来时进行读取，如果缓冲池为空，那会等待缓冲池数据，这是读操作真正耗时的地方。</p>
...
<p>Redis 消息队列不支持多播，为了让 Redis 也支持多播，Redis 使用了一个新的数模块，PubSub，客户端发起订阅命令后，Redis 会立即给予一个反馈消息通知订阅成功。因为有网络传输延迟，在 subscribe 命令发出后，需要休眠一会，再通过 get_message 才能拿到反馈消息。客户端接下来执行发布命令，发布了一条消息。同样因为网络延迟，在 publish 命令发出后，需要休眠一会，再通过 get_ message 才能拿到发布的消息。如果当前没有消息，get_message 会返回空，告知当前没有消息，所以它不是阻塞的。</p>
...
<p>Redis 是一个内存数据库，而内存资源又是非常宝贵的，Redis 可谓是精打细算了，为了避免过大的内存空间消耗，Redis 对对象不同的大小而决定使用存储方式也是不相同的。Redis 对数据量较小的数据结果会采用压缩列表的存储结构进行存储。</p>
<p>好比如，HashMap 是一种二维结构，但是如果数据量比较小的时候，HashMap 是以 ziplist 的存储形式在内存中进行存储的，这是因为压缩列表能够起到节省空间的作用，并且在数据量小的时候，采用遍历一维数组的方式比通过 key-value 获取要快。</p>
...
<p>如果你执行 flushdb，然后再观察内存会发现内存确实被回收了。原因是所有的 key 都干掉了，大部分之前使用的页面都完全干净了，会立即被操作系统回收。Redis 虽然无法保证立即回收已经删除的 key 的内存，但是它会重用那些尚未回收的空闲内存。</p>
<h4 id="内存分配算法" tabindex="-1"><a class="header-anchor" href="#内存分配算法"><span>内存分配算法</span></a></h4>
<p>目前 Redis 可以使用 jemalloc(facebook) 库来管理内存，也可以切换到 tcmalloc(google)。因为 jemalloc 相比 tcmalloc 的性能要稍好一些，所以 Redis 默认使用了 jemalloc。</p>
...
<h3 id="cap-理论" tabindex="-1"><a class="header-anchor" href="#cap-理论"><span>CAP 理论</span></a></h3>
<p>CAP 分布式领域的基石，因为分布式是将机器隔离了，相互的通信只能依靠网络，不过网络其实是不可靠的，出错的概率时常是有的，所以 CAP 理论就为我们定下了标准，即想成为一个合格的分布式系统，你得符合 CAP 理论中的至少两项</p>
<ul>
...
<h3 id="redis-的主从数据一致性" tabindex="-1"><a class="header-anchor" href="#redis-的主从数据一致性"><span>Redis 的主从数据一致性</span></a></h3>
...
<p>快照同步是一个非常消耗资源的操作，它会先在主库通过 bgsave 将当前内存的数据全部快照到磁盘文件，然后再快照文件的内容全部传到从节点。从节点将快照文件接受完毕后，立即执行一次全量加载，加载之前先要将当前内存的数据清空，加载完毕后通知主节点继续进行增量同步。</p>
<p>但是主从快照同步的过程中，主节点对 buffer 的输入是不会停止的，所以如果快照同步速率过慢，主节点仍会覆盖了还未进行增量同步的新数据，那就又会使快照同步开启了，如此循环了，快照同步就停不下来了。</p>
<p>所以一定要注意 buffer 大小的合理设置，避免快照复制的死循环</p>
...
<li>读写分离
<ol>
<li>主服务器只处理写请求，从服务器通过复制功能同步主服务器数据，只处理读请求，以此提升 Redis 服务能力</li>
<li>我们还可以通过复制功能来让主服务器免于执行持久化操作：只要关闭主服务器的持久化功能，然后由从服务器去执行持久化操作即可。</li>
</ol>
</li>
<li>数据容灾
<ol>
<li>由于从服务器与主服务器数据保持同步，一旦主服务器宕机，可以立即将请求切换到从服务器，从而避免 Redis 服务中断。</li>
</ol>
</li>
</ol>
<h3 id="info-指令" tabindex="-1"><a class="header-anchor" href="#info-指令"><span>info 指令</span></a></h3>
<p>info 指令能够帮助我们在很多情况下查看 Redis 或者 Redis 主从的运行状态，其中 info 指令提供了 9 大模块，直接输入 info 指令的话会获取所有的状态信息，当然我们也可以通过指定模块查看具体的状态信息，比如 info memory 查看内存统计数据</p>
<ul>
...
<p>Stats 可以查看到 Redis 实例的统计数据，比如每秒处理客户端连接数</p>
...
<p>复制积压缓冲区的信息可以通过 info replication 查看</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>复制积压缓冲区大小非常重要，它严重影响到主从复制的效率。当从库因为网络原因临时断开了主库的复制，然后网络恢复了，又重新连上的时候，这段断开的时间内发生在 master 上的修改操作指令都会放在积压缓冲区中，这样从库可以通过积压缓冲区恢复中断的主从同步过程</p>
...
<p>当然我们可以通过 info stats 来查看半同步的失败次数来判断是否需要进行缓冲区的扩大</p>
...
<p>Sentinel 是 Redis 提供的一种 Redis 的集群模式，目的在于能够帮助 Redis 切换主从，当主机发生意外宕机时，Sentinel 会从从节点中选出较为合适的从节点作为新的主节点继续提供服务。</p>
...
<p>Cluster 是 Redis 钦点的集群解决方案，Cluster 是去中心化的，每个节点负责整个集群的一部分数据，每个节点负责的数据多少可能不一样，他们相互连接组成一个对等的集群，它们之间通过一种特殊的二进制协议相互交互集群信息。</p>
...
<p>如果客户端根据本地的节点映射表进行节点信息发送，但是该节点发现指令的 key 所在的槽位并不归自己管理，这时它会向客户端发送一个特殊的跳转指令携带目标操作的节点地址，告诉客户端去连这个节点去获取数据。</p>
...
<p>Redis Cluster 提供了工具 redis-trib 可以让运维人员手动调整槽位的分配情况，redis-trib 是通过组合各种原生的 Redis Cluster 指令来实现槽位调整的</p>
...
<p>而在迁移过程中，客户端对数据的访问会有点不一样，因为各自都存在部分 key，所以客户端会先访问旧节点，旧节点有 key 则正常处理，如果没有 key 的话，那说明 key 可能在新节点，也能可能不存在该 key，所以旧节点会向客户端返回一个-ASK targetNodeAddr 的重定向指令。客户端收到这个重定向指令后，先去目标节点执行一个不带任何参数的 asking 指令，然后在目标节点再重新执行原先的操作指令</p>
<p>但是为什么是不带 asking 指令？</p>
...
<p>而从库不具备过期策略，从库对过期的处理是被动的。主库在 key 到期时，会在 AOF 文件里增加一条 del 指令，同步到所有的从库，从库通过执行这条 del 指令来删除过期的 key</p>
...
<p>对于超出 maxmemory 时，Redis 提供了几种可选策略来让用户自己决定如何腾出内存空间来继续提供写服务：</p>
<ul>
<li>
<p>noeviction</p>
...
<li>
<p>volatile-ttl </p>
<p>跟上面一样，除了淘汰的策略不是 LRU，而是 key 的剩余寿命 ttl 的值，ttl 越小越优先被淘汰。</p>
</li>
<li>
<p>volatile-random</p>
...
<h4 id="传统-lru-算法" tabindex="-1"><a class="header-anchor" href="#传统-lru-算法"><span>传统 LRU 算法</span></a></h4>
...
<h4 id="redis-小改-lru-算法" tabindex="-1"><a class="header-anchor" href="#redis-小改-lru-算法"><span>Redis 小改 LRU 算法</span></a></h4>
<p>Redis 使用的是一种近似 LRU 算法，它跟 LRU 算法还不太一样。之所以不使用 LRU 算法，是因为需要消耗大量的额外的内存，需要对现有的数据结构进行较大的改造。</p>
<p>近似 LRU 算法即在现有数据结构的基础上使用随机采样法（二八）来淘汰元素，能达到和 LRU 算法非常近似的效果。Redis 为实现近似 LRU 算法，它给每个 key 增加了一个额外的小字段，这个字段的长度是 24 个 bit，也就是最后一次被访问的时间戳。</p>
<p>Redis 的淘汰属于惰性淘汰，当 Redis 执行写操作时，发现内存超出 maxmemory，就会执行一次 LRU 淘汰算法。这个算法也很简单，就是随机采样出 5(可以配置) 个 key，然后淘汰掉最旧的 key，如果淘汰后内存还是超出 maxmemory，那就继续随机采样淘汰，直到内存低于 maxmemory 为止。</p>
...
<p>虽然说删除指令 del 非常之快，可以做到直接释放对象内存，但是如果要删除的 key 是一个非常大的对象，那么删除操作也会使得单线程的 Redis 出现卡顿的情况</p>
...
<p>主线程执行了 unlink 后，会将关于这个 key 的内存回收操作存储到一个异步队列（需要线程安全）当中，由后台的清理线程从异步队列当中取出并执行。</p>
...
<h3 id="异步-aof" tabindex="-1"><a class="header-anchor" href="#异步-aof"><span>异步 AOF</span></a></h3>
<p>Redis 需要每秒一次(可配置)同步 AOF 日志到磁盘，确保消息尽量不丢失，需要调用 sync 函数，这个操作会比较耗时，会导致主线程的效率下降，所以 Redis 也将这个操作移到异步线程来完成。执行 AOF Sync 操作的线程是一个独立的异步线程，和前面的懒惰删除线程不是一个线程，同样它也有一个属于自己的任务队列，队列里只用来存放 AOF Sync 任务。</p>
